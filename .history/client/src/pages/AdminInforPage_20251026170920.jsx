...
function AdminInfoPage() {
  ...
  useEffect(() => {
    ...
    api.get(`/users/${userId}`)
      .then(res => {
        console.log("API /users/:id", res.data);
        setInfo(res.data); // Receive data
      })
      .catch(err => {
    ...
    <div className="bg-gray-800 p-8 rounded-lg shadow-xl text-white w-full max-w-2xl animate-fadeIn">
      ...
      {info ? (
        <div className="space-y-4 text-lg">
          ...
          <p>
            <span className="font-semibold text-gray-300 w-32 inline-block">Họ và tên:</span> 
            {info.full_name}
          </p>
          // (Đây là code trong file AdminInfoPage.jsx của bạn)

<p>
  <span className="font-semibold text-gray-300 w-32 inline-block">Quyền hạn:</span> 
  {info.role_id === 1 ? 'Quản trị viên' : (info.role_id === 2 ? 'Giáo viên' : 'Học sinh')}
</p>
        </div>
      ) : (
